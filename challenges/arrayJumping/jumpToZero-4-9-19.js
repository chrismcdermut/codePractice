...
const jumpToZero = (array, currentIndex) => {
  ...
  const JumpRightTarget = currentIndex + array[currentIndex];
  console.log('JumpRightTarget');
  console.log(JumpRightTarget);
  const JumpLeftTarget = currentIndex - array[currentIndex];
  ...
  } else if (JumpLeftTarget >= 0 && JumpRightTarget > array.length) { /* only left possible */
    console.log('jumpLeft');
    jumpToZero(array, JumpLeftTarget);
  } else if (JumpRightTarget < array.length && JumpLeftTarget >= 0) { /* left and right possible */
    console.log('get in here??');
    directionObject[currentIndex]
      ? (directionObject[currentIndex] = 'r')
      : (directionObject[currentIndex] = 'l');
    const directonTarget = directionObject[currentIndex] === 'r' ? JumpRightTarget : JumpLeftTarget;
