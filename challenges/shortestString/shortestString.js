...
function removeSection(string, K) {
  ...
  for (let i = 0; i < string.length; i += 1) {
    ...
    const uniqueCharCounterObj = {};
    const inspectionSection = string.substring(i, i + K);
...
function condenseString(string) {
  ...
  for (let i = 0; i < string.length; i += 1) {
    if (currentElement === '') {
      /* initialize current element(eg, character) for first iteration */
      currentElement = string[i];
      currentElementCount += 1;
    }

    if (string[i + 1] === currentElement) {
      /* if next element is same as current element, increment counter */
      currentElementCount += 1;
      continue;
    }

    /* else push it counter and it's element to string */
    const newElement = currentElementCount === 1
      ? currentElement
